...
/*
 * VENDORED VERBATIM. Everything below this comment is the verified source of
 * contracts/Delay.sol behind the Zodiac Delay mastercopy
 * 0xd54895B1121A2eE3f37b502F507631FA1331BED6 (zodiac-modifier-delay v1.0.1),
 * which both live mainnet Delay proxies (0x0C19d8A4..., 0x80Ce5a0d...)
 * delegate to. Fetched from Sourcify; no line has been changed.
 *
 * Unlike certora/helpers/DelayTarget.sol this is the real contract, not a
 * model: it inherits @gnosis.pm/zodiac's Modifier, so executeNextTx really
 * does run Module.exec, which really does call the installed guard. Every
 * dependency it imports (@gnosis.pm/zodiac 1.0.1, @gnosis.pm/safe-contracts
 * Enum/IERC165, @openzeppelin/contracts-upgradeable 4.3.1) is byte-identical
 * to this repo's node_modules, so it compiles against them unmodified.
 *
 * It cannot share a scene with RolesHarness (two Modifier/Module/Guardable
 * bases under the same names), which is why the Roles specs use DelayTarget
 * and the Delay specs use this file.
 *
 * Deployed with solc 0.8.0, optimizer off; compiled here under the repo's
 * solc 0.8.6. Both are 0.8.x checked arithmetic with the same ABI coder.
 */
